...
fHandle = @(x) qcfitfunc(x,inParams);%
% x is standardized, that is 0<=x(i,j)<=1 inside the search space
% if not, qcfitfunc will return infty
...
for lpruns = 1:nRuns   
    ...
    phaseVec = qcCoefs(1)*inParams.dataX + qcCoefs(2)*inParams.dataXSq + qcCoefs(3)*inParams.dataXCb;%
    estSig = sin(2*pi*phaseVec);%
