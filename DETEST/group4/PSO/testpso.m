function outResults = testpso(inParams,psoParams,nRuns)
...
fHandle = @(x) testfitfunc(x,inParams);
...
for lpruns = 1:nRuns   
    ...
    % Signal should be nomalized
    estSig = testNorSig(inParams.dataX,qcCoefs,inParams.sampFreq,1);
    % Estimation of amplitude
    estAmp = innerprodpsd(inParams.dataY,estSig,inParams.sampFreq,inParams.psdVals);
