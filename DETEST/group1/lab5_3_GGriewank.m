function [fitVal,varargout] = lab5_3_GGriewank(xVec,params)
%A benchmark test function for CRCBPSO
%F = lab5_3_GGriewank(X,P)
%Compute the Generalized Griewank fitness function for each row of X.  The fitness
...
for lpc = 1:nrows
    ...
    % functions
        x = xVec(lpc,:);
        temp=1;
        for indexx=1:length(x)
            temp=temp*cos(x(indexx)/sqrt(indexx));
        end
        fitVal(lpc) = sum(x.^2)/4000 - temp +1;
    end
end
