function outResults = CGNqcpso(inParams,psoParams,nRuns)
...
% 'psd': is the PSD for colored noise. The psd should be specified at
%         the positive DFT frequencies corresponding to the length of signal and
%         sampling frequency Fs.
% 'sampFreq': is Sampling frequency 
%
%The fields of O are:
...
fHandle = @(x) CGNqcfitfunc(x,inParams);
...
for lpruns = 1:nRuns   
    ...
    % normalize estSig with SNR =1 according to the given PSD
    [NorEstSig,~] = normsig4psd(estSig,inParams.sampFreq,inParams.psd,1);
    estAmp = innerprodpsd(inParams.dataY,NorEstSig, inParams.sampFreq,inParams.psd);  
%     estAmp = inParams.dataY*estSig(:);
    estSig = estAmp*NorEstSig;
